...
describe("SpaceUseCases", () => {
  describe("create", () => {
    ...
    it("throws NotFoundError when place does not exist", async () => {
      ...
      await expect(
        useCases.create({ placeId: "ghost", name: "X", capacity: 5, opensAt: "08:00", closesAt: "18:00" })
    ...
    it("throws ValidationError when opensAt >= closesAt", async () => {
      ...
      await expect(
        useCases.create({ placeId: "place-1", name: "X", capacity: 5, opensAt: "18:00", closesAt: "08:00" })
    ...
    it("throws ValidationError when opensAt equals closesAt", async () => {
      ...
      await expect(
        useCases.create({ placeId: "place-1", name: "X", capacity: 5, opensAt: "09:00", closesAt: "09:00" })
      ).rejects.toThrow(ValidationError);
    });
  });
});
