...
import { ConflictError, NotFoundError, ValidationError } from "../../../src/domain/errors/domain.errors";
...
function makeReservationRepo(overrides: Partial<ReservationRepository> = {}): ReservationRepository {
  return {
...
function makeSpaceRepo(overrides: Partial<SpaceRepository> = {}): SpaceRepository {
  return {
    ...
    findAll: jest.fn().mockResolvedValue({ data: [], meta: { total: 0, page: 1, pageSize: 20, totalPages: 0 } }),
    findByPlaceId: jest.fn().mockResolvedValue({ data: [], meta: { total: 0, page: 1, pageSize: 20, totalPages: 0 } }),
...
describe("ReservationUseCases", () => {
  describe("create", () => {
    ...
    it("throws NotFoundError when space does not exist", async () => {
      ...
      await expect(
        useCases.create({ spaceId: "ghost", clientEmail: "c@t.com", reservationDate: "2025-12-01", startTime: "09:00", endTime: "11:00" })
    ...
    it("throws ValidationError when startTime >= endTime", async () => {
      ...
      await expect(
        useCases.create({ spaceId: "space-1", clientEmail: "c@t.com", reservationDate: "2025-12-01", startTime: "11:00", endTime: "09:00" })
    ...
    it("throws ConflictError when schedule overlaps an existing reservation", async () => {
      ...
      await expect(
        useCases.create({ spaceId: "space-1", clientEmail: "c@t.com", reservationDate: "2025-12-01", startTime: "10:00", endTime: "12:00" })
    ...
    it("throws ConflictError when client exceeds 3 reservations per week", async () => {
      ...
      await expect(
        useCases.create({ spaceId: "space-1", clientEmail: "c@t.com", reservationDate: "2025-12-01", startTime: "14:00", endTime: "16:00" })
      ).rejects.toThrow(ConflictError);
    });
